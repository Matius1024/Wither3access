...
class CR4IngameMenu extends CR4MenuBase
{
	...
	private var m_w3AccessMenuData		: CScriptedFlashArray;
	private var m_w3AccessOptionsData	: CScriptedFlashArray;
	private var m_w3AccessOptionsDepth	: int;
	private var m_w3AccessBackHandled	: bool;
	private var m_w3AccessBackHandledAt	: int;
	...
	event  OnClosingMenu()
	{
		...
		m_w3AccessBackHandled = false;
		m_w3AccessBackHandledAt = 0;
		W3Access_MenuReset();
	...
	event  OnItemActivated( actionType:int, menuTag:int ) : void
	{
		...
		if (!(m_w3AccessOptionsDepth > 0 && (actionType == IGMActionType_MenuHolder || actionType == IGMActionType_MenuLastHolder)))
		{
			W3Access_ItemActivated(actionType, menuTag);
		}
		if (ignoreInput)
		{
		...
		else
		{
			...
			case IGMActionType_MenuHolder:
				...
				if (m_w3AccessOptionsDepth == 0)
				{
					W3Access_MenuPush();
					W3Access_MenuSubItemsReady(m_w3AccessMenuData, "", menuTag, isMainMenu ? "Menu glowne" : "Menu pauzy");
				}
	...
	{		
		m_w3AccessOptionsDepth += 1;
		m_w3AccessBackHandled = false;
		m_w3AccessBackHandledAt = 0;
		W3Access_MenuPush();
		W3Access_MenuSubItemsReady(m_w3AccessOptionsData, id, menuTag, "Opcje");
	...
	protected function HandleW3AccessOptionBack() : void
	{
		m_w3AccessBackHandled = true;
		m_w3AccessBackHandledAt = theGame.GetLocalTimeAsMilliseconds();
		if (m_w3AccessOptionsDepth > 1)
		{
			m_w3AccessOptionsDepth -= 1;
			W3Access_MenuPop();
		}
		else if (m_w3AccessOptionsDepth == 1)
		{
			RestoreW3AccessRootMenu();
		}
		else
		{
			W3Access_MenuPop();
	...
	event  OnOptionValueChanged(groupId:int, optionName:name, optionValue:string)
	{
		...
		{
			W3Access_OptionValueChangedByName(optionName, optionValue);
		...
		groupName = mInGameConfigWrapper.GetGroupName( groupId );
		W3Access_OptionValueChanged(groupId, optionName, optionValue);
	...
	event  OnNavigatedBack()
	{
		...
		theGame.SetHDRMenuActive(false);
		if (m_w3AccessBackHandled && theGame.GetLocalTimeAsMilliseconds() - m_w3AccessBackHandledAt < 500)
		{
			m_w3AccessBackHandled = false;
		}
		else
	...
	protected function showOptionsPanel() : void
	{
		...
		m_w3AccessOptionsData = l_DataFlashArray;
		m_w3AccessOptionsDepth = 1;
		m_w3AccessBackHandled = false;
		m_w3AccessBackHandledAt = 0;
		W3Access_MenuPush();
		W3Access_MenuItemsReady(m_w3AccessOptionsData, "Opcje");
	...
	function PopulateMenuData()
	{
		...
		m_w3AccessMenuData = l_DataFlashArray;
		m_w3AccessOptionsDepth = 0;
		m_w3AccessBackHandled = false;
		m_w3AccessBackHandledAt = 0;
		W3Access_RootMenuItemsReady(l_DataFlashArray, isMainMenu ? "Menu glowne" : "Menu pauzy");
	...
	protected function SendImportSaveData()
	{
		...
		W3Access_MenuItemsReady(dataFlashArray, "Import zapisu");
	...
	protected function SendLoadData():void
	{
		...
		else
		{
			...
			W3Access_MenuItemsReady(dataFlashArray, "Wczytaj gre");
	...
	protected function SendSaveData():void
	{
		...
		isShowingSaveList = true;
		W3Access_MenuItemsReady(dataFlashArray, "Zapisz gre");
	...
	protected function SendNewGamePlusSaves():void
	{
		...
		else
		{
			m_initialSelectionsToIgnore = 1;
			OnPlaySoundEvent( "gui_global_panel_open" );
			W3Access_MenuItemsReady(dataFlashArray, "Nowa gra plus");
	...
	protected function SendInstalledDLCList():void
	{
		...
		W3Access_MenuItemsReady(dataArray, "Zainstalowane dodatki");
	...
	protected function SendControllerData():void
	{
		...
		W3Access_MenuItemsReady(dataFlashArray, "Pomoc kontrolera");
	...
	protected function SendKeybindData():void
	{
		...
		W3Access_MenuItemsReady(dataFlashArray, "Przypisanie klawiszy");
